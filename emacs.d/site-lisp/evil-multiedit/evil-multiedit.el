...
;;;###autoload
(defun evil-multiedit-skip-and-next()
  (interactive)
  (evil-multiedit-toggle-or-restrict-region)
  (evil-multiedit-next))

;;;###autoload
(defun evil-multiedit-skip-and-prev()
  (interactive)
  (evil-multiedit-toggle-or-restrict-region)
  (evil-multiedit-prev))
...
(when evil-multiedit-dwim-motion-keys
  ...
  (let ((map evil-multiedit-state-map))
    ...
    (define-key map "t"         #'evil-multiedit-skip-and-next)
    (define-key map "T"         #'evil-multiedit-skip-and-prev)
    (define-key map "p"         #'evil-multiedit--paste-after)
    (define-key map "P"         #'evil-multiedit--paste-before)
    (define-key map "n"         #'evil-multiedit-next)
    (define-key map "N"         #'evil-multiedit-prev)
    (define-key map (kbd "C-g") #'evil-multiedit-abort)
