...
(defvar type 'pelican
  "Type of blog backend, options :pelican")
  ...
  (let* ((blog-backend (plist-get plist keyword))
    ...
    (mapcar (lambda (file)
              ...
              (let ((info (funcall read-info file)))
                ...
                (list (plist-get info :title) (plist-get info :publish) (plist-get info :category) (plist-get info :date) file)
                )
              ) (funcall scan-posts))
    ))
  ...
  (defun -read-org-info (post)
    ...
    (with-temp-buffer
      ...
      (let ((info nil))
        ...
        (plist-put info :category
                   (s-trim (-default
                            (s-chop-prefix "#+CATEGORY:" (car (s-match "^#\\+CATEGORY:.*?\n" (buffer-string))))
                            "")))
        (plist-put info :date
                   (s-trim (-default
